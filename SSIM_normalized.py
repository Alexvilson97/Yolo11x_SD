...
real_folder = r"Images\fog_Highway\RD"
synthetic_folder = r"Images\fog_Highway\SD"
output_folder = r"Images\fog_Highway\SSIM_outputs"
...
# Initialize lists to store component averages and SSIM values
...
for idx, (real_file, synthetic_file) in enumerate(zip(real_images, synthetic_images)):
    ...
    # Compute the SSIM score for this pair
    ssim_score = np.mean((2 * mu_x * mu_y + C1) * (2 * sigma_xy + C2)) / (
    ...
    # Save chart with updated naming
    frame_chart_path = os.path.join(output_folder, f"SSIM_Component_Comparison_{real_file}_vs_{synthetic_file}.png")
